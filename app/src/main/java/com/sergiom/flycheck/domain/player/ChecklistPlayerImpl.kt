...
class ChecklistPlayerImpl @Inject constructor() : ChecklistPlayer {
    ...
    override fun reset() { _state.value = null }

    override fun next() {
        val s = _state.value ?: return
        if (s.total == 0) return
        if (s.cursor < s.total - 1) _state.value = s.copy(cursor = s.cursor + 1)
    }

    override fun prev() {
        val s = _state.value ?: return
        if (s.total == 0) return
        if (s.cursor > 0) _state.value = s.copy(cursor = s.cursor - 1)
    }
    ...
    override fun jumpTo(globalIndex: Int) {
        ...
        if (bounded != s.cursor) _state.value = s.copy(cursor = bounded)
    }
    ...
    override fun jumpToSection(sectionIndex: Int) {
        ...
        _state.value = s.copy(cursor = firstPending ?: start)
    }
}
